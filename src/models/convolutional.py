...
class ConvModel(nn.Module):
    ...
    def __init__(self, 
        ...
        if not list(self.return_feature_map.values())[-1]:
            raise ValidationError('There is no output required for the last convolutional block. Consider changing return_feature_map to True')
        ...
        if len(hidden_layers) > 0:
            ...
            if (len(blocks_kwargs) > 1) and (sum(self.return_feature_map.values()) > 1):
                raise ValidationError(f'hidden_layers are specified, while more than 1 feature maps are required to be used later.')
        ...
        if len(hidden_layers) > 0:
            ...
            if all([len(model_kwargs['blocks_kwargs']) > 1 for model_kwargs in conv_models_kwargs.values()]) and \
               any([sum(v.values()) > 1 for v in self.return_feature_map.values()]):
                raise ValidationError(f'hidden_layers are specified, while more than 1 feature maps are required to be used later for one of the models.')
